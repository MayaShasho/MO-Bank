...
const Confirmation = () => {
    ...
    const handleResend = async () => {
        ...
        try {
            setIsResend(true);
            const responseResend = await fetch('http://localhost:8080/signup/resend', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="FormContainer">
            <form className="MainForm" onSubmit={handleConfirmationSubmit}>
                ...
                <p>A confirmation code has been sent to {email}. <br /><br />Please enter it below:</p>
                <PinInput
                    length={6}
                    focus
                    type="numeric"
                ...
                {isResend ? <p className='resendText'>check your email again.</p> : null}
                <ErrorMessage error={error} />
                <button className="SubmitButton" type="submit" disabled={confirmationCode.length !== 6}>
                    Confirm
                </button>
            </form>
        </div>
